...
public class PrecomputedTest {
    ...
    public void basicTest() {
        ...
        Precomputed<String> precomputed = Precomputed.monitorByKeys(
                configuration,
                new Precomputed.Precomputer<String>() {
                    @Override
                    public String compute(Configuration config) {
                        return config.getString("a") + "--" + config.getString("b") + "--" + config.getInt("c", 0);
                    }
                },
                "a", "b");
    ...
    public void refuseNullKeys() {
        Precomputed.monitorByKeys(new BaseConfiguration(), new Precomputed.Precomputer<Object>() {
            @Override
            public Object compute(Configuration configuration) {
                return null;
            }
        }, "a", null);
    }
    ...
    public void nullTest() {
        ...
        Precomputed<String> precomputed = Precomputed.monitorByKeys(
                ...
                new Precomputed.Precomputer<String>() {
                    @Override
                    public String compute(Configuration config) {
                        return String.valueOf(config.getString("a")) + "-" +
